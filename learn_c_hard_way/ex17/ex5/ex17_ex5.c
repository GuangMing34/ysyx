...
#define db_row_size(db)  (db->header.max_data * 2 + sizeof(int)*2 + sizeof(char)*32) /* id + set + name + email */
...
{
    printf("%d %s %s %s\n",
            addr->id, addr->info, addr->city, (addr->info + max_data));
}
...
void Database_set(struct Connection *conn, int id, const char *name, const char *email, const char *city)
{
    ...
    int str_len = strlen(city) > (CITY_NAME_MAX_LEN - 1) ? (CITY_NAME_MAX_LEN - 1) : strlen(city);
    char *res = strncpy(addr->city, city, str_len);
    if(!res) die("Email copy failed", conn);

    str_len = strlen(name) > (max_data_size - 1) ? (max_data_size - 1) : strlen(name);
    res = strncpy(addr->info, name, str_len);
...
int main(int argc, char *argv[])
{
    ...
    switch(action) {
        ...
        case 's':
            if(argc != 7) die("Need id, name, email, city to set", conn);
